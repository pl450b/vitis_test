...
static int SpiSlavePolledSetup(XSpi *SpiInstancePtr, u16 SpiDeviceId);
static int SpiSlavePolledExecute(XSpi *SpiInstancePtr, u8 data);
...
int main(void)
{
	 ...
	 */
	Status = SpiSlavePolledSetup(&SpiInstance, SPI_DEVICE_ID);
	if (Status != XST_SUCCESS) {
		xil_printf("Spi slave setup failed\r\n");
		return XST_FAILURE;
	}

	while(1)
	{
		Status = SpiSlavePolledExecute(&SpiInstance, 0x7e);
		if (Status != XST_SUCCESS) {
				xil_printf("Spi slave setup failed\r\n");
				return XST_FAILURE;
		}
...
//	Status = SpiSlavePolledExecute(&SpiInstance, 0x7e);
//	if (Status != XST_SUCCESS) {
//			xil_printf("Spi slave transfer 1\r\n");
//			return XST_FAILURE;
//	}
//	Status = SpiSlavePolledExecute(&SpiInstance, 0x7e);
//	if (Status != XST_SUCCESS) {
//			xil_printf("Spi slave transfer 2\r\n");
//				return XST_FAILURE;
//	}
}
...
*
******************************************************************************/
int SpiSlavePolledSetup(XSpi *SpiInstancePtr, u16 SpiDeviceId)
{
...
int SpiSlavePolledExecute(XSpi *SpiInstancePtr, u8 data)
{
	//int Status;
	u32 ControlReg;
	WriteBuffer[0] = data;
	 ...
	 */
	ControlReg = XSpi_GetControlReg(SpiInstancePtr);
	XSpi_SetControlReg(SpiInstancePtr, ControlReg & ~XSP_CR_ENABLE_MASK);
